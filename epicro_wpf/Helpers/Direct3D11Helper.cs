...
namespace epicro_wpf.Helpers
{
    ...
    public static class Direct3D11Helper
    {
        ...
        public static IDirect3DDevice CreateDirect3DDevice(IntPtr nativeDevicePtr)
        {
            if (nativeDevicePtr == IntPtr.Zero)
                throw new ArgumentNullException(nameof(nativeDevicePtr), "Native Direct3D device pointer가 null입니다.");

            // P/Invoke를 통해 WinRT용 Direct3D device를 생성합니다.
            uint hr = CreateDirect3D11DeviceFromDXGIDevice(nativeDevicePtr, out IntPtr winrtDevicePtr);
            if (hr != 0)
            {
                throw new Exception("CreateDirect3D11DeviceFromDXGIDevice 호출 실패. HRESULT: 0x" + hr.ToString("X"));
            }

            var device = Marshal.GetObjectForIUnknown(winrtDevicePtr) as IDirect3DDevice;
